...
<div style="display:none;max-height:0;overflow:hidden;opacity:0;color:transparent;">
    Referral update: {{ $transaction->coupon->code }} was used.
</div>
                            ...
                            <tr>
                                <td style="padding:16px 22px 22px 22px;">
                                    <div style="font-family:Inter,Segoe UI,Arial,sans-serif;font-size:13px;line-height:20px;color:#4B5563;">
                                        Hi {{ $transaction->coupon->referral_email }},
                                        <br /><br />
                                        Your coupon <span style="color:#111827;font-weight:800;">{{ $transaction->coupon->code }}</span> was used for a ticket purchase.
                                        <br />
                                        {{--Reward: <span style="color:#111827;font-weight:800;">{{referralReward}}</span>--}}
                                    </div>

                                    {{--<div style="margin-top:14px;">
                                        <a href="https://example.com" style="font-family:Inter,Segoe UI,Arial,sans-serif;font-size:13px;line-height:18px;font-weight:800;color:#4F8A92;text-decoration:none;">View referrals →</a>
                                        <!-- Replace the URL above with {{referralDashboardUrl}} when rendering/sending this template. -->
                                    </div>--}}

                                    <div style="margin-top:14px;font-family:Inter,Segoe UI,Arial,sans-serif;font-size:12px;line-height:18px;color:#6B7280;">
                                        Questions? Contact <a href="mailto:{{ config('mail.support_email') }}" style="color:#4F8A92;text-decoration:none;font-weight:800;">{{ config('mail.support_email') }}</a>.
                                    </div>
                                </td>
                            </tr>
                        </table>
